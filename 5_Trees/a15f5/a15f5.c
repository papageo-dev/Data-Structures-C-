...
int main(){
    ...
    CreateBST(&Root); //DHMIOURGIA DDA

    printf("Enter numbers to insert into tree (-1 to stop)\n");

    /*EISAGWGH TOU PRWTOU STOIXEIOU STO DDA*/
    printf("Enter number: ");
    scanf("%d", &num);
    RecBSTInsert(&Root, num);

    /*EISAGWGH STOIXEIWN STO DDA, MEXRI NA DOTHEI TO -1*/
    while (num!=-1){
            printf("Enter number: ");
            scanf("%d", &num);
            RecBSTInsert(&Root, num);
    ...
    /*EMFANISH TOU PLHTHOUS TWN DEKSIWN KOMBWN/PAIDIWN TOU DDA(Root).*/
    printf("\nRightNodeCount = %d\n", RightNodeCount(Root));
...
/*SYNARTHSH: EPISTROFH TOU PLHTHOUS TWN DEKSIWN KOMBWN/PAIDIWN ENOS DDA(Root).*/
int RightNodeCount(BinTreePointer Root)
{
    int lSum, rSum;
    lSum=rSum=0;

    if (BSTEmpty(Root)){ //AN TO DDA EINAI ADEIO...
        return 0;
    }
    if (!BSTEmpty(Root->LChild)){ //AN YPARXEI STOIXEIO STON ARISTERO KOMBO/PAIDI TOU DDA...
        lSum=RightNodeCount(Root->LChild);
    }
    if (!BSTEmpty(Root->RChild)){ //AN YPARXEI STOIXEIO STON DEKSIO KOMBO/PAIDI TOU DDA...
        rSum=RightNodeCount(Root->RChild)+1;
    }
        return lSum+rSum; //EPISTROFH TOU PLHTHOUS
}
