...
typedef char BinTreeElementType;  //ALLAGH TUPOU SE char
...
int main(){
    ...
    /*DHMIOURGIA KENOU DENTROU(tree)*/
    CreateBST(&tree);

    /*EISAGWGH ARITHMOU GRAMMATWN(letters) POU THA EISAXTHOUN STO DENTRO(tree)*/
    printf("Give the number of letters? ");
    scanf("%d", &letters);

    /*PARAGWGH n(letters) TYXAIWN KEFALAIWN GRAMMATWN(Uppercase) KAI EISAGWGH TOUS STO DDA(tree)*/
    GenerateBST(&tree, letters);

    /*EMFANISH OLWN TWN STOIXEIWN TOU DENTROU(tree)*/
    printf("\n");
...
/*SYNARTHSH: PARAGWGH n TYXAIWN KEFALAIWN GRAMMATWN(Uppercase) KAI EISAGWGH TOUS STO DDA(*Root)*/
void GenerateBST(BinTreePointer *Root, int n)
{
    int i;
    BinTreeElementType Item;

    srand(time(NULL)); //ARXIKOPOIHSH XRONOU SYSTHMATOS, GIA NA EXW DIAFORETIKA GRAMMATA KATHE FORA

    for (i=0; i<n; i++){
        Item=((rand()%(90-65))+65);  //PARAGWGH TYXAIWN GRAMMATWN(A-Z)
        BSTInsert(Root, Item);  //EISAGWGH GRAMMATWN STO DDA
    }
    /*TO DDA DEN THA PERILAMBANEI OLA TA GRAMMATA POU DHMIOURGHTHIKAN STHN DIADIKASIA GenerateBST, GIATI KAPOIA APO TA GRAMMATA
    PARAXTHIKAN PANW APO MIA FORA KAI H YLOPOIHSH THS BSTInsert DEN EPITREPEI THN EISAGWGH IDIWN STOIXEIWN STO DDA.*/
}
